...
data_raw[sapply(data_raw, is.character)] <- lapply(data_raw[sapply(data_raw, is.character)],as.factor)
...
data$year[which(is.na(data$year) & !is.na(data$eventDate))] <- as.numeric(
  format(
    data$eventDate[which(
      is.na(data$year) & !is.na(data$eventDate))],
    '%Y'))
...
data$month[which(is.na(data$month) & !is.na(data$eventDate))] <- as.factor(
  format(
    data$eventDate[which(
      is.na(data$month) & !is.na(data$eventDate))],
    '%m'))
...
```{r}
# ---------------
data_vis <- data[which(data$year > 2014 ), ]
...
# plot all months ----
ggplot(data_vis, aes(x = month , y = family, color = order)) +
  geom_line() +
  geom_point() +
