...
# levels(data$month) = c("Jan","Feb","Mar","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec")
...
top_spp_by_month <- function(m){
  spp_month <- data$species(data[,"month"== m])
  spp_top_month <- spp_month %>%
    group_by(species) %>%
    tally() %>%
    top_n(10)
...
tapply(data$species,data$month,top_spp_by_month(x))
