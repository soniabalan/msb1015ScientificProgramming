...
data[sapply(data, is.character)] <- lapply(data_raw[sapply(data, is.character)],as.factor)
...
### Doesn't work with month as factor
# top_spp_month <- function(m){
#   spp_top_month <- data_vis[data_vis$month == m,] %>%
#   group_by(species) %>%
#   summarize(n = n()) %>%
#   arrange(desc(n)) %>%
#   top_n(10,n)
...
#   top_spp_month <- top_spp_by_month(level)
  ...
  # p <- ggplot(top_spp_month, aes(x = species, y = n)) +
  #   geom_bar(stat="identity") + 
...
# plot all months ----
ggplot(data_vis, aes(month , y = order)) +
  geom_line() +
  geom_point() +
...
process_month <- function(month_data) {
  spp_top_month <- month_data %>%
    group_by(species) %>%
    summarize(n = n()) %>%
    arrange(desc(n)) %>%
    top_n(10, n)
  
  # Create a factor variable for 'species' to specify the order
  spp_top_month$species <- factor(spp_top_month$species, levels = unique(month_data$species))
  
  return(spp_top_month)
}

# Split the data frame by the "month" column
data_split <- split(data, data$month)
result_list <- lapply(data_split, process_month)
result_df <- do.call
# recreate a month column
result_df$month <- sapply(strsplit(rownames(result_df), "\\."), function(x) as.numeric(x[1]))

ggplot(result_df, aes(month , y = n, color = species)) +
  geom_line() +
  # geom_point() +
  # theme(legend.position="none") +
