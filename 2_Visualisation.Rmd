...
Map View
```{r}
ro.map <- get_openstreetmap(bbox= c(left =  min(data_vis$decimalLongitude), bottom =  min(data_vis$decimalLatitude), right =  max(data_vis$decimalLongitude), top = max(data_vis$decimalLatitude)),
