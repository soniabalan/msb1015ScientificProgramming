...
```
  ...
  city/county database from https://github.com/romania/localitati
    - filter based on locality
```{r}
############################################################
'RUN THESE LINES TO GO THROUGH THE PROCESS OF IDENTIFYING THE MISSING PROVINCES'
'This process is very time consuming, even when parallelized on 4 cores'
...
# check distribution of longitude values per province
histograms_lon <- cities_list %>%
  group_by(stateProvince) %>%
  ggplot(aes(x = decimalLongitude)) +
  geom_histogram(binwidth = 0.5) +
  facet_wrap(~stateProvince) +
  labs(title = "Longitude Distribution by Region")
histograms_lon

# check distribution of latitude values per province
histograms_lat <- cities_list %>%
  group_by(stateProvince) %>%
