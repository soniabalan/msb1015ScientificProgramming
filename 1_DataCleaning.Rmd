...
```{r}
'If you could install \'rgeos\', uncomment these lines to run the label replacement yourself. Otherwise, the stateProvince labels were exportet previously and can be loaded directly (in the next code section)'
...
# 
# ##############################################
# 'Add stateProvince labels based on coodinates'
...
#   data$stateProvince[i] <- region$stateProvince
# 
# # intermediary save
# write.csv(data$stateProvince, "geography_labels.csv", row.names=FALSE)
```
  geographical labels upload (if 'rgeos' is not available for install)
```{r}
stateProvince_labels <- read.csv("geography_labels.csv")
data$stateProvince <- stateProvince_labels
...
# sanity checks
print(sum(is.na(data$genus))) # there might still be NA genus entries
...
data_clean <- data %>% select(c("gbifID",
                                "order",
                                "family",
                                "genus",
                                "species",
...
data_clean <- data_clean[indiv_missingness$total<5,]
