...
# Create a "dictionary" of Polygon objects
province_list <- unique(coord_by_province$stateProvince)

prov_boundaries <- list()

# Calculate perimeter for each cloud of points
for (province in province_list) {
  province <- coord_by_province[coord_by_province$stateProvince == state_name, ]
  
  coord <- province[, c("lon_list", "lat_list")]
  points <- SpatialPoints(coord)
  convex_hull <- gConvexHull(points)
  
  # Add the boundary to the list
  prov_boundaries[[state_name]] <- convex_hull
}

# Visualize the boundaries (optional)
plot(prov_boundaries)



# creates a polygon list
state_polygons <- lapply(province_list, function(state) {
  state_data <- coord_by_province[coord_by_province$stateProvince == state, ]
  PolygonCoords <- cbind(state_data$lon_list, state_data$lat_list)
  Polygons(list(Polygon(PolygonCoords)), ID = as.character(state))
})
# creates a SpatialPolygons object
states_sp <- SpatialPolygons(state_polygons)
# removes overalp between areas
states_sp_buffered <- gBuffer(states_sp, width = 0.5)
...
states_tree <- gUnaryUnion(states_sp, id = data.frame(stateProvince = province_list))



# Loop through the missing_indices and use spatial index to find the state

for (i in length(cities_sp)) {
  point <- cities_sp[i, ]
  hits <- over(point, states_tree)

  if (!is.null(hits)) {
    test$stateProvince[missing_indices[i]] <- hits
  }
}
...
#   if (length(region$stateProvince)==1){
#     test$stateProvince[i] <- region$stateProvince
#   }
# }

#################################################
'Paralelized Version' # doesn't work...
...
q <- opq(osm_query)
osm <- osmdata_sf(q)
coords <- st_coordinates(result$osm_polygons)
